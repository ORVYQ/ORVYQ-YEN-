...
async function search(query, key, perPage) {
  ...
  const response = await fetch(url, {
    headers: { Authorization: key, "user-agent": "ORVYQ-demand-footage/1.0" },
...
function pick(videos, usedIds, item) {
  ...
  for (const video of videos) {
    ...
    const files = (video.video_files || []).filter((file) =>
      file.file_type === "video/mp4" && file.width >= 1280 && file.height >= 720 && file.width > file.height && allowedHost(new URL(file.link).hostname)
    );
...
async function probe(file) {
  const { stdout } = await execFileAsync("ffprobe", ["-v", "error", "-select_streams", "v:0", "-show_entries", "format=duration,format_name:stream=codec_name,width,height", "-of", "json", file]);
  const data = JSON.parse(stdout);
  const stream = data.streams?.[0] || {};
  return { duration: Number(data.format?.duration || 0), container: data.format?.format_name, codec: stream.codec_name, width: Number(stream.width || 0), height: Number(stream.height || 0) };
}

async function acquireOne(dir, item, key, usedIds, policy) {
  const id = sceneId(item.scene_id);
  const queries = [...new Set([...(item.queries || []), ...(item.fallback_queries || [])].map(String).map((q) => q.trim()).filter(Boolean))];
  let selected;
  let selectedQuery;
  for (const query of queries) {
    selected = pick(await search(query, key, Number(policy.results_per_query || 30)), usedIds, item);
    if (selected) { selectedQuery = query; break; }
  }
  if (!selected) throw new Error(`${id}: no unique eligible Pexels clip found`);
  ...
  const requested = Math.min(Number(policy.maximum_output_clip_seconds || 16), Number(item.min_duration_seconds || 8) + 4, Number(selected.video.duration));
  const mediaResponse = await fetch(selected.rendition.link, { redirect: "follow", signal: AbortSignal.timeout(180000) });
  if (!mediaResponse.ok || !String(mediaResponse.headers.get("content-type") || "").includes("video")) throw new Error(`${id}: invalid media response`);
  ...
  try {
    await execFileAsync("ffmpeg", ["-y", "-i", source, "-t", String(requested), "-vf", `scale='min(${MAX_WIDTH},iw)':-2`, "-c:v", "libx264", "-preset", "medium", "-crf", "23", "-pix_fmt", "yuv420p", "-movflags", "+faststart", "-an", output], { maxBuffer: 20 * 1024 * 1024 });
  } finally {
    await fs.rm(source, { force: true });
  }
  const info = await probe(output);
  if (info.width < 1280 || info.height < 720 || info.width <= info.height || info.duration + 0.05 < Number(item.min_duration_seconds || 8)) throw new Error(`${id}: compacted media failed HD/duration validation`);
  const buffer = await fs.readFile(output);
  const digest = hash(buffer);
  const provenance = {
    stable_asset_id: shortHash(`${dir}:${id}:${selected.video.id}`), scene_id: id, provider: "pexels", provider_asset_id: String(selected.video.id),
    source_page_url: selected.video.url, license_url: PEXELS_LICENSE, creator: selected.video.user?.name || null, attribution_required: false,
    search_query: selectedQuery, editorial_role: item.role || "context", editorial_note: item.editorial_note || null,
    narration_anchor: item.editorial_assignment?.narration_anchor || null, target_claim_id: item.editorial_assignment?.claim_id || null,
    target_slice_index: item.editorial_assignment?.slice_index ?? null, evidence_use_forbidden: true,
    duration: info.duration, actual_duration_seconds: info.duration, width: info.width, height: info.height, codec: info.codec,
    actual_width: info.width, actual_height: info.height, actual_codec: info.codec, actual_container: info.container,
    byte_size: buffer.length, sha256: digest, actual_sha256: digest, transformed_for_edit: true,
    selected_rendition_id: String(selected.rendition.id), selected_reason: "NARRATION_ANCHORED_UNIQUE_HD_CONTEXT",
    approved_for_final_edit: true, human_review_status: "PENDING_FULL_LENGTH_REVIEW", downloaded_at: new Date().toISOString(),
  };
  await writeJsonAtomic(`${output}.provenance.json`, provenance);
  return { path: relative, scene_id: id, provider_asset_id: String(selected.video.id), role: item.role || "context" };
}

async function reusable(dir, record, id) {
  ...
  return { path: record.path, scene_id: id, provider_asset_id: String(provenance.provider_asset_id), role: record.role || provenance.editorial_role || "context" };
}
...
export async function materializeAssignments(dir, plan, records) {
  ...
  for (const item of items) {
    ...
    if (item.editorial_assignment.replace_graphic_break && !oldGraphic && oldFootage?.asset !== record.path) throw new Error(`${target}: expected graphic target is missing`);
    editorial.footage_assignments[claimId] ||= {};
    editorial.footage_assignments[claimId][index] = {
      asset: record.path, trimInRatio: Number(item.editorial_assignment.trim_in_ratio || 0), motion: item.editorial_assignment.motion || "hold",
      role: item.editorial_assignment.role || item.role || "context", semantic_anchor: item.editorial_assignment.narration_anchor,
    ...
    if (oldGraphic) {
      ...
      if (!Object.keys(editorial.graphic_break_assignments[claimId]).length) delete editorial.graphic_break_assignments[claimId];
      replaced += 1;
    }
    if (!editorial.full_footage_pool.includes(record.path)) editorial.full_footage_pool.push(record.path);
  }
  editorial.generation_policy = "demand-led narration-anchored footage acquisition; exact downloaded paths replace only explicit graphic targets; no fixed clip-count completion and no silent fallback";
  editorial.last_supplemental_footage_materialization = { generated_at: new Date().toISOString(), assignments: items.length, replaced_graphics: replaced };
  await writeJsonAtomic(file, editorial);
  return { assignments: items.length, replaced_graphics: replaced };
}
...
export async function acquireDemandFootage(projectId) {
  ...
  if (plan.project_id !== projectId || !Array.isArray(plan.assets) || !plan.assets.length) throw new Error("Invalid footage acquisition plan");
  ...
  const usedIds = new Set((previous.records || []).map((record) => String(record.provider_asset_id || "")).filter(Boolean));
  ...
  for (const item of plan.assets) {
    const id = sceneId(item.scene_id);
    const old = await reusable(dir, previousByScene.get(id), id);
    if (old) { records.push(old); usedIds.add(old.provider_asset_id); reused += 1; }
    else { records.push(await acquireOne(dir, item, key, usedIds, plan.policy || {})); acquired += 1; }
  }
  await writeJsonAtomic(path.join(dir, "assets", "local_assets.json"), { schema_version: 1, assets: records.map(({ path: assetPath }) => ({ path: assetPath })) });
  await writeJsonAtomic(runtimeFile, {
    schema_version: "1.2-incremental-demand-led", project_id: projectId, generated_at: new Date().toISOString(), source: "pexels",
    license_url: PEXELS_LICENSE, planned_asset_count: plan.assets.length, acquired_this_run: acquired, reused_existing: reused, records,
    pass: records.length === plan.assets.length,
  });
  const materialized = await materializeAssignments(dir, plan, records);
  return { project_id: projectId, acquired, reused, total: records.length, capacity, ...materialized };
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const args = parseArgs(process.argv.slice(2));
  const projectId = args["project-id"] || process.env.ORVYQ_PROJECT_ID;
  acquireDemandFootage(projectId).then((result) => printJson({ ok: true, ...result })).catch((error) => {
