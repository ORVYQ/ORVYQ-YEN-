...
import { projectRoot, readJson, pathExists } from "./fs-utils.mjs";
import {
...
test("shot schema visual_role enum matches the shared canonical role vocabulary", async () => {
  const schema = await readJson(path.join(projectRoot, "schemas", "shot.schema.json"));
  assert.deepEqual(
...
test("every committed motion hook uses canonical roles before full-production planning", async () => {
  ...
  for (const entry of await readdir(projectsDir, { withFileTypes: true })) {
    ...
    const hookPath = path.join(projectsDir, entry.name, "direction", "motion_hook.json");
