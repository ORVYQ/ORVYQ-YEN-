...
Approval is bound to the downloaded asset SHA-256, a contact-sheet SHA-256,
the exact claim, narration anchor and semantic rationale. A provider asset
rejected in any scene is globally excluded from acquisition and backfill.

`scripts/orvyq_acquire_footage_demand.mjs` reuses the 20 validated assets already in the repository, excludes all of their provider IDs, downloads only missing scene IDs, merges the runtime manifest, resolves each downloaded hash path into `editorial_asset_plan.json`, and removes only the declared graphic targets. Re-running it is idempotent.
