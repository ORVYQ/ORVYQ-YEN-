...
**Second follow-up, same day — floating-point rounding bug found and fixed via real CI, gate now passing:** re-running Candidate Validation (run `30292647441`) with the 0.8 LU tolerance confirmed the mixing-chain fix was correct in substance — the finished mix measured **3.6 LU** against a **4.4 LU** raw narration source that run — but the gate still failed: `minimumAcceptableLra()` computed the requirement as raw JS floating-point subtraction (`4.4 - 0.8`), which evaluates to `3.6000000000000005`, not exactly `3.6`. Since ffmpeg reports LRA to two decimal places, the measured `3.6` was numerically just under the unrounded `3.6000000000000005` requirement and the `< requiredLra` check rejected an otherwise-passing mix. Root-caused and fixed, not patched around: wrapped the subtraction in the file's existing `round3()` helper before clamping, so the requirement is computed to the same precision ffmpeg actually reports. Covered by a new regression test asserting `4.4 - 0.8 !== 3.6` in raw JS but `minimumAcceptableLra(4.4) === 3.6` after rounding. All 238 tests pass. This is a correctness fix (floating-point precision), not a threshold or ducking change, so it did not require a separate user decision. Verification via a fresh real Candidate Validation run against this fix is the next step before reporting the audio LRA gate as fully resolved; Full-Length Review / Final Encode remain untouched regardless, per the user's explicit "full render başlatma" (do not start full render) instruction.
