...
- `config/editorial_asset_plan.json`: removed slice 7's illustrative graphic-card entry for CLM_004 (`"2027 TRIAL, THEN ASSESSMENT"`), freeing exactly one slice back to its native evidence kind. That slice's real width (~7.98s, per this claim's own real narration timing) is comfortably above the pipeline's 4s minimum image-evidence hold, so no other slice boundary needed to move.
...
Freed slice 7 was chosen because its illustrative text (`"2027 TRIAL, THEN ASSESSMENT" / "A larger production-chain trial precedes any technical and economic verdict"`) was already paraphrasing exactly what `EVID_JAMSTEC_2027_TRIAL_DIAGRAM` shows as JAMSTEC's own real figure — the fix replaces a hand-written description with the actual official document, not a blind index/keyword swap.
